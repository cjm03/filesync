...
static void sha256_write_byte_block(sha256_t* p) {
    ...
    unsigned i;
    for (i = 0; i < 16; i++) {
        data32[i] = ((uint32_t)(p->buffer[i * 4]) << 24) + ((uint32_t)(p->buffer[i * 4 + 1]) << 16) + ((uint32_t)(p->buffer[i * 4 + 2]) << 8) + ((uint32_t)(p->buffer[i * 4 + 3]));
    }
...
void sha256_final(sha256_t* p, unsigned char* digest) {
    ...
    while (curBufferPos != (64 - 8)) {
        curBufferPos &= 0x3F;
        if (curBufferPos == 0) sha256_write_byte_block(p);
